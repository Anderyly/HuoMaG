...
class Log
{
    ...
    public static function error($msg, $level = 'ERROR')
    {
        ...
        if (is_dir($error)) :
            $str = '[Time]: ' . date('Y-m-d H:i:s') . ' [Leve]:' . $level . ' [Message]:' . $msg . ' [IP]:' . Ip::get() . "\r\n";
    ...
    public static function visit($mode, $controller, $action)
    {
        ...
        if (is_dir($error)) :
            $str = '[Time]: ' . date('Y-m-d H:i:s') . ' [Mode]:' . $mode . ' [Controller]:' . $controller . ' [Action]:' . $action . ' [IP]:' . Ip::get() . ' [Param]:' . self::getParam() . "\r\n";
            file_put_contents($dest, $str, FILE_APPEND | LOCK_EX);
        endif;
