layui.extend({}).define(['layer', 'upload'], function(exports) {
	...
	Class.prototype.init = function() {
		...
		if (multiple) {
			...
			upload.render($.extend({
				...
				choose: function(obj) {
					//layer.open({type: 3, icon: 1});
					layer.msg('<p>上传中...</p>',{icon: 16,shade: 0.5,time:false});
					var files = this.files = obj.pushFile(); //将每次选择的文件追加到文件队列
	...
	Class.prototype.uploadFile = function(filess, filekey, fileCount, data) {
		...
		multipleFileKeyArray.push(this.options.prefixPath + new Date().getTime() + random_string(15) + get_suffix(data.file.name));
